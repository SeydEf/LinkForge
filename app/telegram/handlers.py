...
async def send_admin_user_panel(
    client, chat_id: int, user_id: int, edit_message_id: int = None
):
    ...
    unl_btn_text = (
        f"⚙️ Set Limit ({default_limit_gb} GB)" if quota_bytes == -1 else "♾️ Unlimited"
    )
    ...
    kbd = InlineKeyboardMarkup(
        [
            ...
            [
                ...
                InlineKeyboardButton(
                    unl_btn_text, callback_data=f"admin_qunl_{user_id}"
                ),
            ],
            [
                InlineKeyboardButton(
                    "📂 View Active Links", callback_data=f"admin_vlinks_{user_id}"
                ),
                InlineKeyboardButton(
                    "🗑️ Delete All Links", callback_data=f"admin_delall_{user_id}"
                ),
            ],
        ]
    )
    ...
    if edit_message_id:
        try:
            await client.edit_message_text(
                chat_id, edit_message_id, panel_text, reply_markup=kbd
            )
...
async def admin_search_cmd(client, message: Message):
    ...
    if len(parts) < 2:
        return await message.reply_text(
            "❗ Use format: `/admin_search <username or name>`"
        )
    ...
    for u in results[:20]:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    button_text, callback_data=f"admin_view_{u['user_id']}"
                )
            ]
        )
...
async def admin_ban_unban_callback(client, callback_query):
    ...
    await callback_query.answer(
        f"User {target_id} has been {action_str}.", show_alert=True
    )
    await send_admin_user_panel(
        client,
        callback_query.message.chat.id,
...
async def refresh_admin_links_view(
    client, chat_id: int, message_id: int, target_id: int
):
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🔙 Back to Profile",
                            callback_data=f"admin_view_{target_id}",
                        )
                    ]
                ]
            ),
    ...
    for f in active_files:
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    button_label, url=f"{BASE_URL}/download/{f['uuid']}"
                ),
                InlineKeyboardButton(
                    "🗑️", callback_data=f"admin_delfile_{f['uuid']}_{target_id}"
                ),
            ]
        )
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                "🔙 Back to Profile", callback_data=f"admin_view_{target_id}"
            )
        ]
    )
    ...
    try:
        await client.edit_message_text(
            chat_id, message_id, response_text, reply_markup=keyboard
        )
    except MessageNotModified:
        pass
    except Exception:
...
async def admin_view_links_callback(client, callback_query):
    ...
    target_id = int(callback_query.matches[0].group(1))
    await callback_query.answer()
    await refresh_admin_links_view(
        client, callback_query.message.chat.id, callback_query.message.id, target_id
    )
...
async def admin_delete_file_callback(client, callback_query):
    ...
    if file_info:
        ...
        await callback_query.answer(
            "⚠️ File already deleted or not found.", show_alert=True
        )
...
async def admin_delall_confirm_callback(client, callback_query):
    ...
    kbd = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "🚨 Yes, Delete All", callback_data=f"admin_delallconf_{target_id}"
                ),
                InlineKeyboardButton(
                    "❌ Cancel", callback_data=f"admin_view_{target_id}"
                ),
            ]
        ]
    )
...
async def admin_delall_execute_callback(client, callback_query):
    ...
    await callback_query.answer(
        f"Successfully deleted {deleted_count} links.", show_alert=True
    )
    await send_admin_user_panel(
        client,
        callback_query.message.chat.id,
...
async def admin_ban_cmd(client, message: Message):
    ...
    try:
        ...
        await message.reply_text(
            f"🚫 User `{target_id}` has been successfully restricted."
        )
    except ValueError:
        await message.reply_text(
            "❌ Missing or invalid Target User ID formatting validation."
        )
...
async def admin_setquota_cmd(client, message: Message):
    ...
    if len(parts) < 3:
        return await message.reply_text(
            "❗ Use format: `/admin_setquota <user_id> <limit_in_MB>` (e.g. 2048 for 2GB, 0 for unlimited)"
        )
    ...
    try:
        ...
        if limit_mb < 0:
            return await message.reply_text(
                "❌ Quota limit must be 0 (unlimited) or a positive integer."
            )

        if not db_user_exists(target_id):
            return await message.reply_text(
                f"❌ User `{target_id}` does not exist in the database."
            )
        ...
        await message.reply_text(
            f"✅ Storage quota for user `{target_id}` updated to {limit_str}."
        )
    except ValueError:
        await message.reply_text(
            "❌ Invalid input parameters. Ensure both User ID and limit are integers."
        )
...
async def admin_delete_cmd(client, message: Message):
    ...
    if not file_info:
        return await message.reply_text(
            "❌ Record identity match reference missing from structural scope."
        )
    ...
    await message.reply_text(
        f"🗑️ Row link tracking metadata entry `{target_uuid}` forced offline."
    )
...
async def admin_broadcast_cmd(client, message: Message):
    ...
    if len(parts) < 2:
        return await message.reply_text(
            "❗ Use format: `/admin_broadcast <text content>`"
        )
    ...
    status = await message.reply_text(
        f"📣 Initializing system global broadcast dispatching to `{len(users)}` metrics..."
    )
    success_count = 0
    ...
    for uid in users:
        try:
            await client.send_message(
                chat_id=uid,
                text=f"📢 **System Broadcast Notification**\n\n{broadcast_text}",
            )
            success_count += 1
    ...
    await status.edit_text(
        f"✅ Dispatch complete. Reached `{success_count}/{len(users)}` destinations successfully."
    )
...
async def clone_link_cmd(client, message: Message):
    ...
    await message.reply_text(
        f"✅ **Link Duplicated successfully!**\nNew Pointer:\n{BASE_URL}/download/{new_uuid}"
    )
...
async def remove_pwd_cmd(client, message: Message):
    ...
    if not file_info or file_info["owner_id"] != message.from_user.id:
        return await message.reply_text(
            "⚠️ Action execution identity confirmation unauthorized."
        )

    db_set_password(target_uuid, password=None)
    await message.reply_text(
        "🔓 **Password removed.** Link accessibility state shifted to global public."
    )


@bot.on_message(
    (filters.command("mylinks") | filters.regex(r"^📂 My Links$")) & filters.private
)
async def my_links(client, message: Message):
    ...
    await message.reply_text(
        f"📂 Active link parameters: **{len(active_files)}** entries verified."
    )
    for f in active_files:
        remaining = RETENTION_SEC - (now - f["upload_time"])
        hrs, mins = int(remaining // 3600), int((remaining % 3600) // 60)
...
@bot.on_message(
    (filters.command("summary") | filters.regex(r"^📊 Summary$")) & filters.private
)
...
async def handle_cancellation(client, callback_query):
    ...
    if task_id in ACTIVE_DOWNLOADS:
        ...
        await callback_query.answer(
            "⚠️ Task reference not found or completed.", show_alert=True
        )
...
async def ask_password(client, callback_query):
    ...
    await callback_query.message.reply_text(
        "🔑 Send me the password configuration string for this link payload, or send /cancel."
    )
...
async def remove_password_callback(client, callback_query):
    ...
    try:
        await callback_query.message.edit_caption(
            caption=success_text, reply_markup=keyboard
        )
    except Exception:
        try:
            await callback_query.message.edit_text(
                text=success_text, reply_markup=keyboard, disable_web_page_preview=True
            )
        except Exception:
            pass
...
async def handle_user_delete_callback(client, callback_query):
    ...
    try:
        ...
        try:
            await callback_query.message.edit_text(
                "🗑️ **This file link has been deleted and is no longer active.**"
            )
        except Exception:
            pass
...
@bot.on_message(
    filters.private
    & filters.text
    & filters.create(lambda _, __, m: m.from_user.id in PENDING_PASSWORD),
    group=-1,
)
async def set_password_handler(client, message: Message):
    ...
    if len(password) < 4:
        ...
        await message.reply_text(
            "⚠️ Threshold requirements dictate minimum length of 4 parameters. Try again."
        )
        message.stop_propagation()
        return
    ...
    await message.reply_text(
        "🔒 **Encryption key active.** Link access validation wall enabled."
    )
...
async def handle_direct_url(client, message: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "📤 Upload to Telegram", callback_data=f"urltg_{choice_id}"
                )
            ],
            [
                InlineKeyboardButton(
                    "🌐 Host on Server", callback_data=f"urlhost_{choice_id}"
                )
            ],
...
async def handle_url_choice_telegram(client, callback_query):
    ...
    if not choice:
        return await callback_query.answer(
            "⚠️ Request state parameter expired.", show_alert=True
        )
    ...
    try:
        await download_url_to_path(
            ...
            original_name,
            keyboard,
            "Downloading for Telegram upload",
        )
        ...
        await client.send_document(
            ...
            progress_args=(task_id, status_msg, original_name, time.time()),
        )
...
async def handle_url_choice_host(client, callback_query):
    ...
    if not choice:
        return await callback_query.answer(
            "⚠️ Request entry timeout expiration context.", show_alert=True
        )
    ...
    try:
        await download_url_to_path(
            ...
            original_name,
            keyboard,
            "Downloading for server hosting",
        )
        if ACTIVE_DOWNLOADS.get(task_id, {}).get("cancelled"):
            raise Exception("Cancelled by user")
        ...
        if os.path.exists(download_path):
            ...
            allowed, quota, used = check_quota(
                callback_query.from_user.id, max(0, actual_size - content_length)
            )
            if not allowed:
                raise Exception(
                    f"Quota exceeded: actual size {human_size(actual_size)} exceeds remaining storage limit"
                )

        await finalize_upload(
            status_msg, original_name, download_path, callback_query.from_user.id
        )
    except Exception as e:
        cleanup_file(download_path)
        if "cancelled by user" in str(e).lower():
