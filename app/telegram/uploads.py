...
from .progress import batch_download_progress, download_progress
    ...
    if is_protected:
            ...
            [
                [
                    InlineKeyboardButton(
                        ...
                        callback_data=f"rmpwd_{file_uuid}",
                    )
                ],
                [
                    InlineKeyboardButton(
                        "📊 View Stats Data", url=f"{BASE_URL}/stats/{file_uuid}"
                    ),
                    InlineKeyboardButton(
                        "🗑️ Delete File", callback_data=f"delete_{file_uuid}"
                    ),
                ],
            ]
        )
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "🔒 Set Security Password", callback_data=f"setpwd_{file_uuid}"
                    )
                ],
                [
                    InlineKeyboardButton(
    ...
    try:
        ...
        try:
            await status_msg.edit_text(
                success_text, disable_web_page_preview=True, reply_markup=keyboard
            )
        except Exception:
...
async def process_media_batch(user_id: int, chat_id: int):
    ...
    status_msg = await bot.send_message(
        chat_id, f"📦 **Processing batch package of {len(messages)} items into zip...**"
    )
    ...
    try:
        for idx, msg in enumerate(messages, 1):
            ...
            actual_path = await bot.download_media(
                msg,
                file_name=d_path,
                progress=batch_download_progress,
                progress_args=(
                    task_id,
                    status_msg,
                    f_name,
        ...
        await status_msg.edit_text(
            "🤐 **Archiving batch contents into system Zip file...**"
        )
...
async def handle_single_media_processing(message: Message):
    ...
    try:
        ...
        await finalize_upload(
            status_msg, file_name, file_path, message.from_user.id, duration
        )
        ...
        async with session.get(url) as response:
            ...
            with open(download_path, "wb") as f:
                async for chunk in response.content.iter_chunked(65536):
                    ...
                    now = time.time()
                    if (
                        now - ACTIVE_DOWNLOADS[task_id]["last_update"] >= 2.0
                        or current_size == total_size
                    ):
                        ACTIVE_DOWNLOADS[task_id]["last_update"] = now
                        elapsed = now - start_time
                        speed = current_size / elapsed if elapsed > 0 else 0
                        bar = (
                            make_progress_bar(current_size, total_size)
                            if total_size
                            else "[------------] ?%"
                        )
                        size_info = (
                            f"{human_size(current_size)} / {human_size(total_size)}"
                            if total_size
                            else human_size(current_size)
                        )
